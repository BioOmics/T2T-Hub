...
# Read ortho info
ortho_path = '/public/workspace/biobigdata/project/t2thub/UserUpload/{}/reference_pep/gene_ortho/{}.gene_ortho.txt'.format(species,species)
try:
    with open(ortho_path) as f:
        ortho_dict = { rec.strip().split()[0]:rec.strip().split()[1] for rec in f.readlines() }
except:
    ortho_dict = dict()
    print("Error parsing ortho info. Skip...")

...
# 判断genome.telo.info是否存在 (Just check if genome.telo.info exists)
...
# 初始化JSON结构 (Initialize JSON structure)
echo '{"keys":["name","start","length","color"], "annots":['

# 处理每个染色体的注释 (Process annotations for each chromosome)
...
# 合并并输出染色体注释 (Merge and output chromosome annotations)
first=1

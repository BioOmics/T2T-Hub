...
if [ ! -s "11.ko00001.filter4Animal.txt" ]; then
    echo "11.ko00001.filter4Animal.txt not found or empty: run 11.ko00001.filter.R firstly"
    exit 1
fi
...
if [ ! -f "org.${j}.eg.db_1.0.tar.gz" ]; then
	python3 /pathway/script4animal/11.getGeneTable.py ${i}
	Rscript /pathway/script4animal/11.OrgDBmaker.R ${i} ${j} ${tax_id}
	cp /pathway/script4animal/11.ko00001.filter4Animal.txt ./org.${j}.eg.db/inst/extdata/ko00001.T2THub.txt
	bash /pathway/script4animal/11.addPathway2R.sh org.${j}.eg.db

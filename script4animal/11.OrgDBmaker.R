...
# 检查是否传递了参数
if (length(args) == 0) {
  stop("Usage: Rscript OrgDBmaker.R Homo_sapiens_CHM13")
}
...
ko2pathway <- read.table("11.ko00001.filter4Animal.txt", header = T, sep = "\t", stringsAsFactors = F, quote = "", check.names = F)[, c(5, 4)]

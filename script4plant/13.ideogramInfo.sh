...
# 读取染色体长度信息到一个数组中 (read chromosome length information into an associative array)
...
# 输出格式化的JSON 结构 (output formatted JSON structure)
echo '{"chrBands":['
...
awk -vOFS="\t" -v lengths="$chr_length_str" 'BEGIN {
  ...
  # 过滤掉起始位置大于1且末尾位置靠近染色体末尾100000以内的候选着丝粒 (Filtering out candidate centromeres that are too close to the chromosome ends)

...
for rec in gff_record:
    ...
    gff_result.append({'gene_ID': gene_ID, 'source_ID': source_ID, 'location': location, 'start': start, 
                       'end': end, 'strand': strand, 'tf_type': tf_type, 'tf_family': tf_family, 
                       'function': function, 'go': go, 'kegg': kegg})


# In[ ]:


# with open('./data/{}.geneSQL.json'.format(species), 'w') as f:
#     json.dump(gff_result, f)

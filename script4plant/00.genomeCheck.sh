...
# 从 metadata.txt 提取 Genome File 和 GFF3 File 路径 (extract paths of Genome File and GFF3 File from metadata.txt)
...
decompress_file() {
    ...
    case $file in
        ...
        *.bz2)
            ...
            ;;
        *)
            echo "$file" # 非压缩文件直接返回 (non-compressed file is returned directly)
            ;;
    esac
}
...
# ===================================
# 检查 genome.gff 和 genome.fa 是否匹配, 以及gff3文件是否符合规范 (check if genome.gff and genome.fa match, and if the gff3 file meets the specifications)
# 1.必须要有gene的feature，2.必须要有ID和Parent属性，3.必须明确正反链，(must have gene feature, must have ID and Parent attributes, must specify strand), 4. feature的起始位置必须小于终止位置，5.gff3中长度必须小于等于fasta文件中的长度，6. 序列名必须一致 (feature start position must be less than end position, length in gff3 must be less than or equal to length in fasta file, sequence names must be consistent)
# 4.feature的起始位置必须小于终止位置，5.gff3中长度必须小于等于fasta文件中的长度，6. 序列名必须一致 (feature start position must be less than end position, length in gff3 must be less than or equal to length in fasta file, sequence names must be consistent)
...
# 如果gapNm大于chrNm*2，或者大于50个，就认为这个基因组不符合T2T的要求，退出 (If gapNm is greater than chrNm*2, or greater than 50, it is considered that the genome does not meet the requirements of T2T, exit)
...
# ===================================
# 检查蛋白质文件是否符合规范 (check if the protein file meets the specifications)
...
# Filter for sequences that are at least minLength long and save their IDs (过滤出长度至少为minLength的序列，并保存它们的ID)
awk -v minLength=${minLength} '{if ($2 >= minLength){print $1}}' genome.size | sort -k1,1V > ids.txt && cat ids.txt

# Check the number of sequences after filtering (检查过滤后的序列数量)
...
# Extract sequences with the filtered IDs from the genome file and save to a new file (从基因组文件中提取具有过滤后ID的序列，并保存到新文件中)
seqkit grep -f ids.txt genome.fa > genome.fa.new

# Filter the GFF3 file to include only entries corresponding to the filtered sequences (过滤GFF3文件以仅包含与过滤后序列对应的条目)
